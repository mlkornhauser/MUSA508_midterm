...
```{r Miami.base, message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}
miami.base <- 
...
stargazer(VarSummary_internal, VarSummary_amenity, VarSummary_census, VarSummary_spatial, 
          type = "text", 
          global.summary = TRUE, 
          title = c("Internal Characteristics", "Amenities & Services", "Census", "Spatial"))
...
1. Assisted Living Facilities - Average Nearest 4 Neighbors: The scatterplot indicates that being farther away from assisted living facilities has a positive relationship with a home's sale price. This suggests that the most expensive homes are not in neighborhoods with a substantial elderly population.

2. Barbershops - Average Nearest 4 Neighbors: The scatterplot indicates that being farther away from barbershops has a positive relationship with a home's sale price. While a barbershop may be considered an essential business, this might indicate a socio-economic or cultural divide in the city. Higher income residents with more expensive home may be more likely to frequent "a salon" instead of a barbershop. A barbershop may also be more common in non-white neighborhoods, which correlates negatively with home prices.
...
ggplot() +
  geom_sf(data = miami.base.sf, fill = "grey40") +
  geom_sf(data = miami.sf, aes(colour = q5(hotel_nn4)), size = .75) +
  scale_colour_manual(values = palette5,
                      labels = qBr(miami.sf, "hotel_nn4"),
                      name = "Avg Distance to 4 NN\n(Quintile Breaks)") +
  labs(title="Nearest Hotels, Miami",
       subtitle = "Figure X.X") + 
  mapTheme()

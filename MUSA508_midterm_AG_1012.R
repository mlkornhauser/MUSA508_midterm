...
# MIAMI BASE
############
# Pull in Miami and Miami Beach boundaries and convert to SF file
miami.base <- 
...
# Pull in housing data and convert to SF file
...
# Plot home sale prices in Miami and Miami Beach
ggplot() +
...
st_drop_geometry(miami.sf) %>% 
  dplyr::select(SalePrice, pctWhite, pctPov) %>% #Choose variables from main dataset
...
numericVars <- 
  ...
  dplyr::select(SalePrice, AdjustedSqFt, Stories, Bed, Bath, bars_nn4, malls_nn4, 
                ...
                TopAvgNhood, TopMedNhood, Zip_33139, neighbor.10M_1320, pctWhite, pctPov) %>%
  na.omit()
...
# k nearest neighbors - create a list of 5 neighbors for each point, relate each sale observation to the neighbors,
# and calculate the average price of a home sale's 5 nearest neighbors
...
ggplot(as.data.frame(moranTest$res[c(1:999)]), aes(moranTest$res[c(1:999)])) +
  ...
  plotTheme()
# The observed I is higher than all of the 999 randomly generated I's, confirming spatial autocorrelation/clustering
# and suggests that variation in price likely related to the spatial process has been omitted from our model
...
reg.nhood <- lm(SalePrice ~ ., data = as.data.frame(miami.training) %>% 
                  dplyr::select(LABEL, SalePrice, marina_nn1, ent_nn4, Age, post2000,
                                AdjustedSqFt, Stories, bars_nn4, malls_nn4, 
                                hotel_nn4, BeachDist, Pool, LuxPool, Dock, Patio,
                                Elevator, ent_nn4, ent_nn3, ent_nn2, ent_nn1, marina_nn4, 
                                marina_nn3, marina_nn2, marina_nn1, restaurants_nn4, restaurants_nn3,
...
bothRegressions <- 
  rbind(
    dplyr::select(miami.test, starts_with("SalePrice"), Regression, LABEL) %>%
